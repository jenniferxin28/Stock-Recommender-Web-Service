...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        def split_list_equally(data_list, num_pieces):
            length = len(data_list)
            part_length = length // num_pieces
            return [data_list[i * part_length:(i + 1) * part_length] for i in range(num_pieces - 1)] + [data_list[(num_pieces - 1) * part_length:]]
        
        nasdaq_symbols = tickers_nasdaq(False)
        my_list = ['W', 'R', 'P', 'Q']
        del_set = []
        sav_set = []
        for symbol in nasdaq_symbols:
            if len( symbol ) > 4 and symbol[-1] in my_list:
                del_set.append(symbol)
            else:
        ...
        for symbol in nasdaq:
            ...
            try:
                ...
                ratio = None
            nasdaq_ratios[symbol] = ratio

        valid_ratios = [ratio for ratio in nasdaq_ratios.values() if ratio is not None]
        ...
        # convert to np array
        valid_ratios = np.array([ratio for ratio in nasdaq_ratios.values() if ratio is not None])
        ...
        for symbol in nasdaq_symbols:
            ...
            # P/E ratio
            pe_ratio = nasdaq_ratios[symbol.replace("-", ".")]
        ...
        for index, row in df_clean.iterrows():
            stock, created = NASDAQStocks.objects.update_or_create(
                symbol=row['Symbol'], 
                defaults={
